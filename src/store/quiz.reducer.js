...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    case actionTypes.LOAD_QUIZ_STATE:
      return {
        ...state,
        ...action.payload,
      };
    ...
    case actionTypes.UPDATE_TEXT_PARAGRAPH:
      ...
      newExerciseArrayUTP[action.payload.exIndex].paragraphs[
        action.payload.pgIndex
    ...
    case actionTypes.ADD_OPTION:
      ...
      newExerciseArrayAO[action.exIndex].options.push({
        id: makeCustomId(5),
        label: 'Option ' + (len + 1),
      });
    ...
    case actionTypes.SET_CORRECT_ANSWER:
      ...
      newExerciseArraySCA[action.payload.exIndex].answer_key[0] =
        action.payload.id;
    ...
    case actionTypes.UPDATE_OPTION:
      ...
      newExerciseArrayUO[action.payload.exIndex].options[
        action.payload.opIndex
    ...
    case actionTypes.REMOVE_OPTION:
      ...
      if (answerIndex !== -1) {
        newExerciseArrayROPT[action.payload.exIndex].options.splice(
          action.payload.opIndex,
          1
        );
        newExerciseArrayROPT[action.payload.exIndex].answer_key.splice(
          answerIndex,
          1
        );
      } else {
    ...
    case actionTypes.REORDER_OPTION:
      ...
      let reorderedOptionArray =
        newExerciseArrayROOPT[action.payload.exIndex].options;
      if (
        !(action.payload.opIndex === 0 && action.payload.offset === -1) &&
        !(
          action.payload.index === reorderedOptionArray.length &&
        ...
        moveSegment(
          reorderedOptionArray,
          action.payload.opIndex,
          action.payload.opIndex + action.payload.offset
        );
    ...
    case actionTypes.REORDER_EXERCISE:
      ...
      if (
        !(action.payload.exIndex === 0 && action.payload.offset === -1) &&
        !(
          action.payload.exIndex === newExerciseArrayRE.length &&
          action.payload.offset === 1
        )
      ) {
        const moveSegment = (array, from, to) => {
        ...
        moveSegment(
          newExerciseArrayRE,
          action.payload.exIndex,
          action.payload.exIndex + action.payload.offset
        );
    ...
    case actionTypes.REMOVE_PARAGRAPH:
      ...
      newExerciseArrayRP[action.payload.exIndex].paragraphs.splice(
        action.payload.pgIndex,
        1
      );
    ...
    case actionTypes.REORDER_PARAGRAPH:
      ...
      let reorderedParagraphArray =
        newExerciseArrayROP[action.payload.exIndex].paragraphs;
      if (
        !(action.payload.pgIndex === 0 && action.payload.offset === -1) &&
        !(
          action.payload.index === reorderedParagraphArray.length &&
          action.payload.offset === 1
        )
      ) {
        const moveSegment = (array, from, to) => {
        ...
        moveSegment(
          ...
          action.payload.pgIndex,
          action.payload.pgIndex + action.payload.offset
        );
    ...
    case actionTypes.UPDATE_ELEMENT:
      ...
      newExerciseArrayUE[action.payload.exIndex].paragraphs[
        action.payload.pIndex
      ].elements[action.payload.elIndex].content = action.payload.content;
    ...
    case actionTypes.TOGGLE_IS_NUMBERED:
      ...
      newExerciseArrayTIN[
        action.payload.exIndex
      ].isNumbered = !newExerciseArrayTIN[action.payload.exIndex].isNumbered;
    ...
    case actionTypes.REMOVE_GAP:
      ...
      let elements =
        newExerciseArrayRG[action.payload.exIndex].paragraphs[
          action.payload.pgIndex
        ].elements;
      ...
      if (gapIndex > 0 && gapIndex < elements.length) {
        if (
          elements[gapIndex - 1].type === 'text_run' &&
          elements[gapIndex + 1].type === 'text_run'
        ) {
          // merging
          const newTextRun = {
            ...
            type: 'text_run',
            content:
              elements[gapIndex - 1].content.trim() +
              ' ' +
              elements[gapIndex + 1].content.trim(),
          };
    ...
    case actionTypes.INSERT_GAP:
      ...
      const initialContent =
        newExerciseArrayIG[action.payload.exIndex].paragraphs[
          action.payload.pgIndex
        ].elements[action.payload.elIndex].content;
      const newElements = [
        {
          id: makeCustomId(5),
          type: 'text_run',
        ...
        },
        {
          ...
          content: initialContent
            .substr(
              action.payload.splitIndex,
              initialContent.length - action.payload.splitIndex
            )
            .trim(),
        },
      ];
      newExerciseArrayIG[action.payload.exIndex].paragraphs[
        action.payload.pgIndex
      ].elements.splice(action.payload.elIndex, 1, ...newElements);
